...
def resnet(nb_classes, depth=20, wide_factor=1, weight_decay=1E-4, sparse_factor=1e-4):
    if depth == 20:
        nb_blocks = [3, 3, 3]
    elif depth == 28:
        nb_blocks = [4, 5, 4]
    ...
    # stage 1
    x = residual_block(y, nb_filters[0], sparse_factor=sparse_factor)
    y = add([x, y])
    ...
    # stage 2
    x = residual_block(y, nb_filters[1], strides=(2, 2), sparse_factor=sparse_factor)
    ...
    for _ in range(nb_blocks[1] - 1):
        x = residual_block(y, nb_filters[1], sparse_factor=sparse_factor)
        y = add([x, y])

    # stage 3
    x = residual_block(y, nb_filters[2], strides=(2, 2), sparse_factor=sparse_factor)
    ...
    y = add([x, y])
    for _ in range(nb_blocks[1] - 1):
        x = residual_block(y, nb_filters[2], sparse_factor=sparse_factor)
        y = add([x, y])
